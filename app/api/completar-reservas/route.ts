...
// Endpoint llamado por cron-job.org cada 15 minutos
// Configuración cron-job.org:
//   URL: https://khepria.app/api/completar-reservas
//   Método: GET
...
function buildResenaHtml(r: any, resenaUrl: string): string {
      ...
      <tr>
        <td style="padding:32px 36px;">
          ...
          <p style="margin:0 0 28px;font-size:15px;color:#4B5563;line-height:1.6;">
            Nos encantaría saber qué te pareció. Solo te llevará 30 segundos y nos ayuda a mejorar.
          </p>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
            <tr>
              <td align="center">
                <a href="${resenaUrl}" style="display:inline-block;padding:15px 36px;background:#111827;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;border-radius:14px;letter-spacing:-0.2px;">
                  Dejar mi valoración →
                </a>
              </td>
            </tr>
...
async function ejecutar(): Promise<NextResponse> {
  ...
  if (errPasadas) {
    console.error('[completar-reservas] error buscando pasadas:', errPasadas)
  ...
  if (errHoy) {
    console.error('[completar-reservas] error buscando hoy:', errHoy)
  }
  ...
  if (errUpdate) {
    console.error('[completar-reservas] error actualizando estado:', errUpdate)
    return NextResponse.json({ error: errUpdate.message }, { status: 500 })
  }

  // ── 4. Sumar puntos de fidelización ───────────────────────────────────────
  ...
  // ── 5. Enviar email de reseña post-reserva ────────────────────────────────
  let resenas = 0
  let erroresResena = 0
  ...
  for (const r of paraCompletar) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const html = buildResenaHtml(r as any, resenaUrl)
    ...
    if (res.ok) {
      await sb.from('reservas').update({ resena_enviada: true }).eq('id', r.id)
      resenas++
    } else {
      const body = await res.json().catch(() => ({}))
      console.error('[completar-reservas] error Resend para reserva', r.id, body)
      erroresResena++
    }
  }

  console.log(`[completar-reservas] ${ids.length} completadas, ${resenas} reseñas enviadas, ${erroresResena} errores`)
  return NextResponse.json({ ok: true, completadas: ids.length, resenas, erroresResena })
}
